...
%However, other divertor diagnostics that have been used as the basis for detachment control like Thomson scattering \cite{eldon_2017_nf} and Langmuir probes \cite{eldon_2021_nme} also suffer from inability to return useful control signals during ELMs.
%The problem is solved by identifying and removing samples that occur during ELMs (in the case of diagnostics like Thomson scattering that return infrequent samples with short averaging windows) or lowpass filtering the signals to minimize the perturbation due to ELMs (can be viable for diagnostics that return frequent samples; in the case of foil bolometers \cite{eldon_2019_nme}, the filtering happens in the sensor itself due to its response time being slow relative to ELMs).
...
Errors incurred by omitting tungsten can be mitigated by the need to keep tungsten at extremely low concentrations to avoid radiative collapse, as it is a very potent core radiator, although concern recovers upon reflecting that tungsten should be strongly compressed into the divertor as an operational goal facilitated by the tendency of high Z species to be compressible. 
\fixme{Make sure the preceding statement is correct and try to get a good reference for it.}
% \cite{wang_2023_sinica}. %but omitting tungsten from boundary models of tungsten devices is often done as it is such a potent core radiator that plasma discharges typically end before tungsten concentrations reach levels where edge radiation is significant, and also the computational cost is much higher for higher Z species.

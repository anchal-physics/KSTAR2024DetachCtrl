...
\textcolor{red}{You should quote the probe spacing to go with 16~mm FWHM. Also, if the probe spacing were also about 16~mm, you only ensure that your max error is no more than 50\%. So be careful about this confidence statement. It seems that significant underestimates could still occur. Even 20\% could be impactful! We are programming Afrac trajectories where the difference between 0.4 and 0.3 matters. Okay, that's 25\%, but still.}
...
\textcolor{red}{Please double check KSTAR clock cycle; I thought it was 50~$\mu$s, not 5.}
...
\textcolor{red}{Maybe mention here that the impurity estimator wasn't connected properly for these experiments/it malfunctioned?}
\textcolor{red}{Consider elaborating on input power. $P_{ohm} \approx I_p \cdot V_{loop}$, which doesn't match more refined estimates for ohmic power. Probably because this simple approximation assumes that all of Ip is inductively driven. PNBI comes from beam on/off command, programmed beam voltage, and beam power vs voltage lookup table. Discrepancies between actual on/off and requested on off are not detected (bad). ECH is purely a feedforward waveform based on what the ech group says they're programming. This is better than the 0 it was before (2022 ppcf paper), but prone to errors keeping it updated and won't detect sources not firing when requested (like nbi).}

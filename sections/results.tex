...
\fixme{LLNL CHECK THIS FOLLOWING STATEMENT PLEASE!!!!}
The insensitive regions are due to the way UEDGE is run on a grid, with many runs at the same density boundary condition.
If this is not resolved within UEDGE or within the neural network itself, an added layer should be added to eliminate these artifacts before the predicted heat flux reaches a controller.
